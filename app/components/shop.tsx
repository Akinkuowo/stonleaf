...
const ShopPage = ({ initialProducts }: ShopPageProps) => {
  ...
  const categories = [
    ...
    { name: 'CANVAS ART', id: 'Canvas Art', label: 'Canvas Art' },
    { name: 'PHOTO ART', id: 'Photo Art', label: 'Photo Art' },
    { name: 'DIGITAL ART', id: 'Digital Art', label: 'Digital Art' },
    { name: 'AR ART', id: 'Ar Art', label: 'AR Art' },
    // Removed 'SCULPTURES' and 'ARTIFACTS' to match header
  ];

  // Use passed products prop
  const artworks = initialProducts.map(product => ({
    ...product,
    title: product.name, // Map name to title for UI consistency if needed, or update UI to use name
    image: product.imageUrl || '/images/placeholder.jpg', // Handle null image
    // category: product.category // Category should match DB values
  }));

  // Filter artworks based on current category
  const filteredArtworks = currentCategory === 'all'
    ? artworks
    : artworks.filter(artwork => artwork.category === currentCategory); // Ensure case sensitivity matches or normalize
  ...
  const handleArtworkHover = (artworkId: string) => {
